...
router.get('/usage', usageRateLimit, authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const sanitizedResponse = {
      usage: {
        totalQueries: Math.max(0, parseInt(String(usage.total_queries || 0), 10)),
        webSearchQueries: Math.max(0, parseInt(String(usage.web_search_queries || 0), 10)),
        deepResearchQueries: Math.max(0, parseInt(String(usage.deep_research_queries || 0), 10)),
        maxTotal: 100,
        maxWebSearch: 20,
        maxDeepResearch: 3
      },
      userApiKey: userApiKey && typeof userApiKey === 'string' && userApiKey.length >= 4 
        ? '***' + userApiKey.slice(-4) 
        : null
    }
...
export const updateUsage = async (userId: string, type: 'total' | 'webSearch' | 'deepResearch' = 'total') => {
  try {
    ...
    // Validate current usage values and prevent overflow
    const currentTotal = Math.max(0, parseInt(String(usage.total_queries || 0), 10))
    const currentWebSearch = Math.max(0, parseInt(String(usage.web_search_queries || 0), 10))
    const currentDeepResearch = Math.max(0, parseInt(String(usage.deep_research_queries || 0), 10))
    
    // Check limits before incrementing
    if (currentTotal >= 1000000) { // Prevent unreasonable values
      throw new Error('Usage limit exceeded')
    }
    
    // Increment counters with bounds checking
    const updates: any = {
      total_queries: Math.min(currentTotal + 1, 1000000),
      updated_at: new Date().toISOString()
    }
    
    if (type === 'webSearch') {
      updates.web_search_queries = Math.min(currentWebSearch + 1, 1000000)
    } else if (type === 'deepResearch') {
      updates.deep_research_queries = Math.min(currentDeepResearch + 1, 1000000)
    }
...
export const hasRemainingQuota = async (userId: string, type: 'total' | 'webSearch' | 'deepResearch' = 'total'): Promise<boolean> => {
  try {
    // Validate inputs
    if (!validateUserId(userId)) {
      console.error('Invalid user ID format in hasRemainingQuota')
      return false
    }
    
    if (!validateUsageType(type)) {
      console.error('Invalid usage type in hasRemainingQuota')
      return false
    }
    ...
    // Validate and sanitize usage values
    const totalQueries = Math.max(0, parseInt(String(usage.total_queries || 0), 10))
    const webSearchQueries = Math.max(0, parseInt(String(usage.web_search_queries || 0), 10))
    const deepResearchQueries = Math.max(0, parseInt(String(usage.deep_research_queries || 0), 10))
    
    switch (type) {
      case 'total':
        return totalQueries < limits.total
      case 'webSearch':
        return webSearchQueries < limits.webSearch
      case 'deepResearch':
        return deepResearchQueries < limits.deepResearch
      default:
        return false
    }
  } catch (error) {
