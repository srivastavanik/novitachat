...
const conversationLimiter = rateLimit({
  ...
  max: 50, // Limit conversation operations per hour
  message: {
...
const searchLimiter = rateLimit({
  windowMs: 1 * 60 * 1000, // 1 minute
  max: 20, // Limit search requests per minute
  message: {
...
// Message management - with message-specific rate limiting
router.get('/conversations/:conversationId/messages', messageLimiter, (req, res) => chatController.getMessages(req, res));
router.post('/conversations/:conversationId/messages', messageLimiter, (req, res) => chatController.sendMessage(req, res));

// Search - with search-specific rate limiting
router.get('/search', searchLimiter, (req, res) => chatController.searchConversations(req, res));

// Archive operations - with conversation rate limiting
